...
type context struct {
	...
	ignorefileext string // Extensions to ignore (CSV)
}
...
func newContext(config Config, r *http.Request) (c *context, err error) {
	...
	// If -ignorefileext is not an empty string, pass it into the context
	if config.IgnoreFileExt != "" {
		c.ignorefileext = config.IgnoreFileExt
	}
